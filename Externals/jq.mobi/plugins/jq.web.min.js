...
(this[g]=f[g]);else return new c(a,f);try{var k=this;this.pagingDiv=this.pagingDiv?document.getElementById(this.pagingDiv):null;this.container.style.overflow="hidden";this.container.style["-webkit-box-orient"]="vertical";this.container.style.display="-webkit-box";this.container.style["-webkit-box-orient"]="vertical";if(this.vertical)this.horizontal=!1;var d=document.createElement("div"),b=Array.prototype.slice.call(this.container.childNodes);this.horizontal?(d.style.display="-webkit-box",d.style["-webkit-box-flex"]=
1):d.style.display="block";i(d).append(b);this.container.appendChild(d);this.el=d;this.refreshItems();d.addEventListener("touchmove",function(a){k.touchMove(a)},!1);d.addEventListener("touchend",function(a){k.touchEnd(a)},!1);d.addEventListener("touchstart",function(a){k.touchStart(a)},!1);k=this;window.addEventListener("orientationchange",function(){k.onMoveIndex(k.carouselIndex,0)},!1)}catch(e){console.log("error adding carousel "+e)}}else alert("Error finding container for carousel "+a)};c.prototype=
...
0}},touchMove:function(a){if(this.movingElement){if(a.touches.length>1)return this.touchEnd(a);var f={x:a.touches[0].pageX-this.startX,y:a.touches[0].pageY-this.startY};if(this.vertical)f={x:0,y:0},this.dy=a.touches[0].pageY-this.startY,this.dy+=this.cssMoveStart,f.y=this.dy,a.preventDefault();else{var g;if(g=!this.lockMove)f=Math.round(Math.atan2(f.y,f.x)*180/Math.PI),f<0&&(f=360-Math.abs(f)),g=f<=215&&f>=155||f<=45&&f>=0||f<=360&&f>=315?!0:!1;if(g)f={x:0,y:0},this.dx=a.touches[0].pageX-this.startX,
this.dx+=this.cssMoveStart,a.preventDefault(),f.x=this.dx;else return this.lockMove=!0}f&&this.moveCSS3(this.el,f)}},touchEnd:function(){if(this.movingElement){var a=!1;try{var f=this.vertical?numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el,null).webkitTransform)).f):numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el,null).webkitTransform)).e);if(f>0)this.moveCSS3(this.el,{x:0,y:0},"300");else{var g=this.vertical?this.dy%this.myDivHeight/this.myDivHeight*-100:this.dx%this.myDivWidth/
this.myDivWidth*-100,k=this.carouselIndex;f<this.cssMoveStart&&g>3?k++:f>this.cssMoveStart&&g<97&&k--;k>this.childrenCount-1&&(k=this.childrenCount-1);k<0&&(k=0);f={x:0,y:0};this.vertical?f.y=k*this.myDivHeight*-1:f.x=k*this.myDivWidth*-1;this.moveCSS3(this.el,f,"150");if(this.pagingDiv&&this.carouselIndex!==k)document.getElementById(this.container.id+"_"+this.carouselIndex).className=this.pagingCssName,document.getElementById(this.container.id+"_"+k).className=this.pagingCssNameSelected;this.carouselIndex!=
k&&(a=!0);this.carouselIndex=k}}catch(d){console.log(d)}this.dx=0;this.movingElement=!1;this.startY=this.dy=this.startX=0;a&&this.pagingFunction&&typeof this.pagingFunction=="function"&&this.pagingFunction(this.carouselIndex)}},onMoveIndex:function(a,f){this.myDivWidth=numOnly(this.container.clientWidth);this.myDivHeight=numOnly(this.container.clientHeight);var g=!1;try{document.getElementById(this.container.id+"_"+this.carouselIndex).className=this.pagingCssName;var d=Math.abs(a-this.carouselIndex),
b=a;b<0&&(b=0);b>this.childrenCount-1&&(b=this.childrenCount-1);var c={x:0,y:0};this.vertical?c.y=b*this.myDivHeight*-1:c.x=b*this.myDivWidth*-1;var h=f?f:50+parseInt(d*20);this.moveCSS3(this.el,c,h);this.carouselIndex!=b&&(g=!0);this.carouselIndex=b;if(this.pagingDiv)document.getElementById(this.container.id+"_"+this.carouselIndex).className=this.pagingCssNameSelected}catch(e){console.log("Error "+e)}g&&this.pagingFunction&&typeof this.pagingFunction=="function"&&this.pagingFunction(currInd)},moveCSS3:function(a,
f,g,k){g=g?parseInt(g):0;k||(k="linear");a.style.webkitTransform="translate"+b+f.x+"px,"+f.y+"px"+d;a.style.webkitTransitionDuration=g+"ms";a.style.webkitBackfaceVisiblity="hidden";a.style.webkitTransformStyle="preserve-3d";a.style.webkitTransitionTimingFunction=k},addItem:function(a){a&&a.nodeType&&(this.container.childNodes[0].appendChild(a),this.refreshItems())},refreshItems:function(){var a=0,f=this,g=this.el;n=g.childNodes[0];for(var d,b=[];n;n=n.nextSibling)n.nodeType===1&&(b.push(n),a++);this.childrenCount=
a;d=parseFloat(100/this.childrenCount)+"%";for(a=0;a<b.length;a++)this.horizontal?(b[a].style.width=d,b[a].style.height="100%"):(b[a].style.height=d,b[a].style.width="100%",b[a].style.display="block");this.moveCSS3(g,{x:0,y:0});this.horizontal?(g.style.width=Math.ceil(this.childrenCount*100)+"%",g.style.height="100%",g.style["min-height"]="100%"):(g.style.width="100%",g.style.height=Math.ceil(this.childrenCount*100)+"%",g.style["min-height"]=Math.ceil(this.childrenCount*100)+"%");if(this.pagingDiv){this.pagingDiv.innerHTML=
"";for(a=0;a<this.childrenCount;a++)g=document.createElement("div"),g.id=this.container.id+"_"+a,g.pageId=a,g.className=a!==this.carouselIndex?this.pagingCssName:this.pagingCssNameSelected,g.onclick=function(){f.onMoveIndex(this.pageId)},b=document.createElement("div"),b.style.width="20px",this.horizontal?(b.style.cssFloat="left",b.innerHTML="&nbsp;"):(b.innerHTML="&nbsp;",b.style.display="block"),this.pagingDiv.appendChild(g),a+1<this.childrenCount&&this.pagingDiv.appendChild(b),b=g=null;this.pagingDiv.style.width=
this.childrenCount*50+"px";this.pagingDiv.style.height="25px"}this.onMoveIndex(this.carouselIndex)}};return c}}()})(jq);(function(i){i.fn.css3Animate=function(c){for(var b,d=0;d<this.length;d++)b=new e(this[d],c);return this.length==1?b:this};i.css3AnimateQueue=function(){return new e.queue};var e=function(){if(window.WebKitCSSMatrix){var c="m11"in new WebKitCSSMatrix?"3d(":"(",b="m11"in new WebKitCSSMatrix?",0)":")",d=function(h,a){this.el=typeof h=="string"||h instanceof String?document.getElementById(h):h;if(!(this instanceof d))return new d(h,a);if(this.el){var f=this;if(a){a.time===void 0&&(a.time=0);if(a.callback){if(this.callback=
...
if(!this.handle)return alert("Could not find handle for drawer -  "+a);var g=this;this.handle.addEventListener("touchmove",function(a){g.touchMove(a)},!1);this.handle.addEventListener("touchend",function(a){g.touchEnd(a)},!1)}catch(b){alert("error adding drawer"+b)}this.zIndex=i(this.el).css("zIndex")}else alert("Could not find element for drawer "+a)};h.prototype={lockY:0,lockX:0,boolScrollLock:!1,currentDrawer:null,maxTop:0,startTop:0,maxLeft:0,startLeft:0,timeMoved:0,vdistanceMoved:0,hdistanceMoved:0,
...
d)}this.lockY=a.touches[0].pageY;this.lockX=a.touches[0].pageX;this.currentDrawer=f;a.preventDefault()}}catch(c){alert("error in drawer start: "+c)}},touchEnd:function(a){if(this.currentDrawer!=null){a.preventDefault();a.stopPropagation();a={x:0,y:0};if(this.direction=="up"||this.direction=="down"){var f=-this.vdistanceMoved,b=Math.ceil(Math.abs(f)/Math.abs(this.maxTop)*100),c=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).f);a.y=b>17?f>0?this.maxTop:0:Math.floor(this.maxTop/
c)>2?0:this.maxTop}else f=-this.hdistanceMoved,b=Math.ceil(Math.abs(f)/Math.abs(this.maxLeft)*100),c=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).e),b>17?a.x=f>0?this.maxLeft:0:Math.floor(this.maxLeft/c)>2?a.y=0:a.x=this.maxLeft;this.el.zIndex=a.y>0||a.x>0?"9999":this.zIndex;this.drawerMove(this.currentDrawer,a,300,"ease-out");this.currentDrawer=null}this.vdistanceMoved=0;d=!1},drawerMove:function(a,f,g,d){g||(g=0);d||(d="linear");a.style.webkitTransform="translate"+
...
c==1?"none":"disc"}}})(jq);(function(i){var e=[];i.fn.scroller=function(b){var d;if(b===void 0&&this.length>0)return e[this[0].id]?e[this[0].id]:null;for(var h=0;h<this.length;h++)d=new c(this[h],b),this[h].id&&(e[this[h].id]=d);return this.length==1?d:this};var c=function(){function b(a,b){var d=document.createElement("div");d.style.position="absolute";d.style.width=a+"px";d.style.height=b+"px";d.style.webkitBorderRadius="2px";d.style.opacity=0;d.className="scrollBar";d.style.background="black";return d}if(window.WebKitCSSMatrix){var d=
